...
export const getAllClientCodesOfPaymentsIn2008 = async () => {
    let res = await fetch("http://172.16.101.146:5475/payments")
...
export const getAllPaymentsFromPayPalEachYear = async () => {
    let res = await fetch("http://172.16.101.146:5475/payments?payment=PayPal")
    let data = await res.json();
    let dataUpdate = [];
    data.forEach(val => {
...
export const getAllClientsWhoPaid = async (code) => {
    let res = await fetch(`http://172.16.101.146:5475/payments?code_client=${code}`)
    let data = await res.json();
    return data;
