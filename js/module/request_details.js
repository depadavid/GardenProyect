export const getAllRequestDetailsByCode = async(product_code)=>{
    let res = await fetch(`http://172.16.101.146:5537/request_details?product_code=${product_code}`);
...
export const getAllRequestDetailsByRequestCode = async(code_request)=>{
    let res = await fetch(`http://172.16.101.146:5537/request_details?product_code=${code_request}`);
    let data = await res.json();
    return data;
}
